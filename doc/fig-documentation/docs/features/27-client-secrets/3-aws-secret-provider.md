...
If `autoCreate` is set to `false`, you only need the `GetSecretValue` permission.

## Error Handling

The provider handles common AWS errors gracefully:

- **ResourceNotFoundException**: Thrown when a secret doesn't exist and auto-create is disabled
