...
- **Automatic Secret Creation**: Optionally creates secrets if they don't exist
...
// Create provider using Application Default Credentials
...
If `autoCreate` is set to `false`, you only need:
...
- **NotFound**: Thrown when a secret doesn't exist and auto-create is disabled
