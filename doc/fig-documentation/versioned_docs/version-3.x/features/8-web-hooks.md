...
| Security Event              | When security-related events occur in Fig (such as successful or failed login attempts), this web hook will be sent. This allows integration with SIEM systems and security monitoring tools. The webhook includes details such as event type, username, success status, IP address, hostname, and failure reason (if applicable). | [Github](https://github.com/mzbrau/fig/blob/main/src/common/Fig.WebHooks.Contracts/SecurityEventDataContract.cs) | /SecurityEvent             |
