...
public class SettingClientFacadeLoadPerformanceTests
{
    ...
    [Test]
    public async Task LoadAllClients_WhenInitializeScriptsFalse_LeavesInitializationPendingUntilInitializeAllClientsAsync()
    {
        var scriptRunner = new Mock<IScriptRunner>();
        scriptRunner
            .Setup(r => r.RunScript(It.IsAny<string?>(), It.IsAny<IScriptableClient>(), It.IsAny<bool>()))
            .Returns(ScriptRunResult.Succeeded("ClientA"));

        var clientWithScript = new SettingClientConfigurationModel(
            "ClientA",
            "Loading...",
            null,
            false,
        ...
        var setting = new StringSettingConfigurationModel(
            new SettingDefinitionDataContract(
                ...
                false,
                typeof(string),
                displayScript: "Timeout.IsEnabled = true;"),
            clientWithScript,
            new SettingPresentation(false));
        clientWithScript.Settings.Add(setting);

        _definitionConverter
            .Setup(converter => converter.Convert(
                It.IsAny<IList<SettingsClientDefinitionDataContract>>(),
                It.IsAny<Action<(string, double)>>()))
            .ReturnsAsync(new List<SettingClientConfigurationModel> { clientWithScript });

        _httpService
            .Setup(service => service.Post("/diagnostics/web-client-load", It.IsAny<object>()))
            .Returns(Task.CompletedTask);

        await _sut.LoadAllClients(initializeScripts: false);

        scriptRunner.Verify(
            r => r.RunScript(It.IsAny<string?>(), It.IsAny<IScriptableClient>(), It.IsAny<bool>()),
            Times.Never);

        await _sut.InitializeAllClientsAsync();

        scriptRunner.Verify(
            r => r.RunScript(
                It.Is<string?>(s => s == "Timeout.IsEnabled = true;"),
                It.IsAny<IScriptableClient>(),
                true),
            Times.Once);

        // Second call is a no-op once initialization has completed.
        await _sut.InitializeAllClientsAsync();
        scriptRunner.Verify(
            r => r.RunScript(It.IsAny<string?>(), It.IsAny<IScriptableClient>(), It.IsAny<bool>()),
            Times.Once);
