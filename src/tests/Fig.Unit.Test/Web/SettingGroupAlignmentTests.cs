...
public class SettingGroupAlignmentTests
{
    ...
    [Test]
    public void ShallUpdateAlignmentWhenGroupValueSetFromManagedSetting()
    {
        var clientA = new SettingClientConfigurationModel("ClientA", "desc", null, false, _scriptRunner);
        var clientB = new SettingClientConfigurationModel("ClientB", "desc", null, false, _scriptRunner);
        ...
        // Simulate a compare/refresh update via the suppressed path
        groupSetting.SetValueFromManagedSetting("200");

        // Alignment should still be recalculated even though propagation was suppressed.
        // Group value is now "200", settingA is still "100", so still misaligned.
        Assert.That(groupSetting.HasMisalignedGroupValues, Is.True,
            "Group should still be misaligned because only the group value changed, not all managed settings");

        // Now align via push which propagates to all managed settings
        groupSetting.PushValueToGroupManagedSettings();
