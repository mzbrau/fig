...
public class SettingCompareServiceTests
{
    ...
    public async Task ShallIncludeLastChangedMetadata()
    {
        ...
        _dataFacade.Setup(f => f.GetLastChangedForAllSettings("ClientA", null))
            .ReturnsAsync(lastChangedEntries);
    ...
    public async Task ShallHandleInstancedClients()
    {
        ...
        SetupLastChangedLookup("ClientA", "Inst1");
    ...
    private void SetupLastChangedLookup(string clientName, string? instance)
    {
        _dataFacade.Setup(f => f.GetLastChangedForAllSettings(clientName, instance))
            .ReturnsAsync((IEnumerable<SettingValueDataContract>?)null);
    }
