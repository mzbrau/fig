...
public class ApiSecretRotationStateServiceTests
{
    ...
    [Test]
    public void PersistedStatusMapping_ShouldRoundTripEnumNames()
    {
        foreach (var status in Enum.GetValues<ApiSecretRotationMigrationStatus>())
        {
            var storageValue = ApiSecretRotationStatusPersistence.ToStorageValue(status);

            Assert.That(storageValue, Is.EqualTo(status.ToString()));
            Assert.That(ApiSecretRotationStatusPersistence.Parse(storageValue), Is.EqualTo(status));
        }
    }
