...
public class SettingLoadTypeAndTimestampTests : IntegrationTestBase
{
    ...
    public async Task ShallHaveValidTimestampAndServerLoadTypeWhenLoadingFromApi()
    {
        ...
        Assert.That(lastUpdateTime, Is.GreaterThanOrEqualTo(beforeLoadTime), 
            "LastFigUpdateUtc should be recent (after test start)");
        
        Assert.That(lastUpdateTime, Is.LessThanOrEqualTo(DateTime.UtcNow.AddSeconds(2)), 
            "LastFigUpdateUtc should be recent (within 2 seconds of now)");
        
        Assert.That(lastUpdateTime.Kind, Is.EqualTo(DateTimeKind.Utc), 
            "LastFigUpdateUtc should be in UTC");
    }
    ...
    public async Task ShallHaveOfflineLoadTypeAndValidTimestampWhenUsingOfflineSettings()
    {
        ...
        var lastUpdateTime = currentSettings.LastFigUpdateUtc!.Value;
        Assert.That(lastUpdateTime, Is.GreaterThanOrEqualTo(beforeOfflineLoadTime), 
            "LastFigUpdateUtc should be recent (updated during offline load)");
    ...
    public async Task ShallUpdateTimestampWhenSettingsReload()
    {
        ...
        Assert.That(updatedSettings.LastFigUpdateUtc!.Value.Kind, Is.EqualTo(DateTimeKind.Utc), 
            "LastFigUpdateUtc should remain in UTC after reload");
    }
    ...
    public async Task ShallMaintainLoadTypeConsistencyAcrossMultipleReloads()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            Assert.That(currentSettings.LastFigUpdateUtc!.Value.Kind, Is.EqualTo(DateTimeKind.Utc), 
                $"LastFigUpdateUtc should be in UTC after reload {i + 1}");
