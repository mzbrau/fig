...
public class DatabaseMigrationService : IDatabaseMigrationService
{
    ...
    public async Task RunMigrationsAsync()
    {
        ...
        try
        {
            ...
            // Try to acquire lock
            if (!await _migrationRepository.TryAcquireMigrationLock())
            {
                _logger.LogInformation("Another instance is running migrations. Waiting for completion...");
                await WaitForMigrationsToComplete();
                return;
            }
            ...
            {
                await _migrationRepository.ReleaseMigrationLock();
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task RunMigrationsInternal()
    {
        ...
        var executedNumbers = new HashSet<int>(executedMigrations.Select(m => m.ExecutionNumber));
    ...
    private async Task ExecuteMigration(IDatabaseMigration migration)
    {
        ...
        try
        {
            ...
            var migrationRecord = new DatabaseMigrationBusinessEntity
            {
                ExecutionNumber = migration.ExecutionNumber,
                Description = migration.Description,
                ExecutedAt = DateTime.UtcNow,
                ExecutionDuration = stopwatch.Elapsed
            };

            await _migrationRepository.RecordMigrationExecution(migrationRecord);
    ...
    private async Task WaitForMigrationsToComplete()
    {
        // Wait for the other instance to complete migrations
        const int maxWaitTimeSeconds = 300; // 5 minutes
        const int checkIntervalMs = 1000; // 1 second
        
        var waited = 0;
        while (waited < maxWaitTimeSeconds * 1000)
        {
            await Task.Delay(checkIntervalMs);
            waited += checkIntervalMs;
            
            // Try to acquire lock to check if migrations are done
            if (await _migrationRepository.TryAcquireMigrationLock())
            {
                await _migrationRepository.ReleaseMigrationLock();
                _logger.LogInformation("Migration lock available. Other instance has completed migrations");
                return;
            }
        }

        _logger.LogWarning("Timed out waiting for other instance to complete migrations. Proceeding anyway");
    }
}
