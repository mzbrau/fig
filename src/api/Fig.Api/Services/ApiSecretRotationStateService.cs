...
public class ApiSecretRotationStateService : IApiSecretRotationStateService
{
    ...
    public async Task<ApiSecretRotationSnapshot> GetSnapshot(bool upgradeLock = false)
    {
        ...
        var status = ApiSecretRotationStatusPersistence.Parse(state?.Status);
    ...
    public async Task<ApiSecretRotationStatusDataContract> GetStatus()
    {
        ...
        {
            Status = ApiSecretRotationStatusPersistence.ToStorageValue(snapshot.Status),
    ...
    public async Task<ApiSecretRotationSnapshot> MarkMigrationStarted()
    {
        ...
        if (state?.Status == ApiSecretRotationStatusPersistence.ToStorageValue(ApiSecretRotationMigrationStatus.MigrationInProgress))
            throw new InvalidOperationException($"API secret migration is already in progress on host {state.StartedByHost}.");

        if (state?.Status == ApiSecretRotationStatusPersistence.ToStorageValue(ApiSecretRotationMigrationStatus.MigrationCompleted))
    ...
    public async Task MarkMigrationCompleted()
    {
        ...
        state.UpdatedAtUtc = DateTime.UtcNow;
        var now = DateTime.UtcNow;
        state.Status = ApiSecretRotationStatusPersistence.ToStorageValue(ApiSecretRotationMigrationStatus.MigrationCompleted);
    ...
    public async Task MarkMigrationFailed(Exception exception)
    {
        ...
        var now = DateTime.UtcNow;
        ClearLiveProgress();
        state.Status = ApiSecretRotationStatusPersistence.ToStorageValue(ApiSecretRotationMigrationStatus.MigrationFailed);
    ...
    {
        state.Status = ApiSecretRotationStatusPersistence.ToStorageValue(ApiSecretRotationMigrationStatus.MigrationInProgress);
        ClearLiveProgress();
