...
public class SettingsService : AuthenticatedService, ISettingsService
{
    ...
    private async Task ApplyDeferredImport(SettingClientBusinessEntity client)
    {
        ...
        {
            var clientToUpdate = deferredImport.Instance is null
                ? client
            ...
            await _settingChangeRecorder.RecordSettingChanges(changes, null, DateTime.UtcNow, clientToUpdate, deferredImport.AuthenticatedUser);
            await _eventLogRepository.Add(_eventLogFactory.DeferredImportApplied(clientToUpdate.Name, clientToUpdate.Instance));
