...
if (!connectionString || connectionString.trim() === '') {{
    ...
    {scriptPropertyName}.ValidationExplanation = '{propertyName} cannot be empty';
}} else {{
    ...
    if (!hasDataSource) {{
        {scriptPropertyName}.IsValid = false;
        {scriptPropertyName}.ValidationExplanation = '{propertyName} must contain a valid Data Source (Server)';
    }} else if (!hasDatabase) {{
        {scriptPropertyName}.IsValid = false;
        {scriptPropertyName}.ValidationExplanation = '{propertyName} must contain either Initial Catalog (Database) or AttachDBFilename';
    }} else {{
...
if (!rows || !Array.isArray(rows)) {{
    ...
    for (var rowIndex = 0; rowIndex < rows.length; rowIndex++) {{
        ...
        var rowMessage = null;
        if (!connectionString || connectionString.trim() === '') {{
            rowMessage = '{fieldName} cannot be empty';
        }} else {{
            var hasDataSource = /(?:server|data source|addr|address|network address)\s*=/i.test(connectionString);
            var hasDatabase = /(?:initial catalog|database)\s*=/i.test(connectionString) ||
                             /attachdbfilename\s*=/i.test(connectionString);
            ...
            if (!hasDataSource) {{
                rowMessage = '{fieldName} must contain a valid Data Source (Server)';
            }} else if (!hasDatabase) {{
                rowMessage = '{fieldName} must contain either Initial Catalog (Database) or AttachDBFilename';
            }}
        }}
